...
export default function StableGoals() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      if (Date.now() - digestingTask.startTime >= digestingTask.duration) {
        const userRef = doc(db, "users", session.docId);
        const earnedHay = digestingTask.reward_hay || 0;
        const earnedSugar = digestingTask.reward_sugar || 0;
        
        await updateDoc(userRef, {
          "currency.hay": increment(earnedHay),
          "currency.sugar": increment(earnedSugar),
          [`stats.${digestingTask.reward_stat}`]: increment(1),
          digestingTask: null
        });
        showToast(`Task Complete! +${earnedHay} Hay, +${earnedSugar} Sugar`);
  ...
  const buyItem = async (item) => {
    if (userData.currency.sugar >= item.price) {
      ...
      showToast("Not enough Sugar!", "error");
    }
  };
  ...
  const contributeToTown = async (buildingId) => {
    ...
    if (userData.currency.hay >= 50) {
      ...
      showToast("Contributed 50 Hay to Town!");
    } else {
      showToast("Not enough Hay!", "error");
    }
  };

  const sendGift = async (targetUserId) => {
    if (userData.currency.sugar >= 10) {
  ...
  const handleWellness = async (mood) => {
    ...
    showToast("Checked in! +Stats, +Sugar, +Hay");
  };
        ...
        {raceMode ? (
          <div className="bg-slate-900 text-white p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden min-h-[60vh]">
             <div className="flex justify-between items-center mb-8 relative z-10">
                <h2 className="text-4xl font-black italic tracking-tighter text-amber-400">Weekly Gallop</h2>
                <div className="flex gap-2">
                   {raceState === 'waiting' && <button onClick={startRace} className="bg-emerald-500 hover:bg-emerald-600 px-6 py-2 rounded-xl font-bold transition-all shadow-lg shadow-emerald-900/50">START RACE</button>}
      ...
      <main className="flex-1 overflow-hidden grid grid-cols-1 lg:grid-cols-12">
        ...
        <section className="lg:col-span-7 bg-[#F0F4F8] flex flex-col h-full overflow-hidden">
          <div className="px-8 pt-8 pb-4 flex gap-4 overflow-x-auto no-scrollbar shrink-0">
             ...
             {townData.buildings.find(b => b.id === 'bus')?.unlocked && <GameButton active={view === 'team'} onClick={() => setView('team')} color="amber" icon={Bus}>Team</GameButton>}
          ...
          <div className="flex-1 overflow-y-auto px-8 pb-8">
                  ...
                  <div key={task.id} className={`bg-white p-5 rounded-2xl border-2 flex justify-between items-center hover:shadow-md transition-shadow ${task.isMain ? 'border-emerald-200 bg-emerald-50/30' : 'border-slate-100'}`}>
                    <div>
                        ...
                        <div className={`${task.isMain ? 'text-emerald-600' : 'text-slate-400'} font-bold text-xs mt-1 flex items-center gap-1`}>
                           <span className="uppercase">{task.type}</span> • +{task.reward_hay} Hay / +{task.reward_sugar} Sugar
                        </div>
                    </div>
                    <GameButton onClick={() => feedHorse(task)} color={task.isMain ? "emerald" : "white"} disabled={!!userData.digestingTask}>🍎</GameButton>
                  </div>
                )) : (
                  <div className="text-center py-20 bg-white rounded-[2.5rem] border-4 border-dashed border-slate-200 h-full flex flex-col justify-center items-center">
            ...
            {view === 'team' && (
               <div className="grid grid-cols-2 gap-4">
                  {teamMembers.filter(m => m.teamCode === userData.teamCode).map(member => (
                     <div key={member.id} className="bg-white p-4 rounded-3xl border-2 border-slate-100 relative group">
                        <div className="text-center mb-2 font-bold text-slate-700">{member.name}</div>
                        <div className="h-24 bg-slate-50 rounded-2xl flex items-center justify-center relative overflow-hidden mb-3">
                           <img src={`/assets/horses/${member.horseColor || 'white'}.png`} className="absolute h-full object-contain" />
                        </div>
                        {member.id !== userData.id && townData.buildings.find(b => b.id === 'gift_shop')?.unlocked && (
                           <button onClick={() => sendGift(member.id)} className="w-full bg-pink-100 text-pink-600 font-bold text-xs py-2 rounded-xl flex items-center justify-center gap-1 hover:bg-pink-200">
                              <Gift size={14} /> Gift (+10 Sugar)
                           </button>
                        )}
                     </div>
                  ))}
